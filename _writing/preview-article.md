...
subtitle: "Why \"explainability\" keeps failing the test of practical ethics, and what a more honest research program might look like."
category: "Article"
date: 2026-05-26
...
authors:
  - { name: "Tanay Nagar", affiliation: "Independent · Madison, WI" }
topics: [AI Ethics, Explainability, Philosophy, Accountability]
tldr: "Explainable AI keeps failing the test of practical ethics not because of bad methods but because the concept itself smuggles in commitments it can't pay for. The argument: justification is not a technical step; trust is a relational achievement between institutions and subjects, not a model property. We can help. We cannot deliver."
cover: /assets/images/writing/cover-test-1.png
cover_alt: "Dotted halftone illustration of two figures"
has_math: false
published: true
resources:
  - { label: "PDF",   url: "#", kind: "paper" }
  - { label: "arXiv", url: "#", kind: "paper" }
  - { label: "DOI",   url: "#", kind: "paper" }
  - { label: "GitHub",   url: "#", kind: "code" }
  - { label: "Notebook", url: "#", kind: "code" }
  - { label: "Dataset",  url: "#", kind: "code" }
  - { label: "YouTube", url: "#", kind: "talks" }
  - { label: "Slides",  url: "#", kind: "talks" }
discussion:
...
[^1]: The argument that follows draws heavily on recent work by Lazar and Vredenburgh — see footnotes 3 and 4 below for the citations.
...
Vredenburgh's recent work[^vredenburgh2023] makes a sharper version of this point. When a decision is contested, the question "why?" is not asking for a causal account. It's asking for a justification that survives the questioner's standards of reasonableness. Two people receiving the same SHAP plot will rationally have different residual questions, because their stake in the decision is different.
...
{% include figure.html
   ...
   caption="**Figure 1:** Tiered explainability framework. As contestability claims become finer-grained, the required form of explanation becomes more procedural — less \"what the model did\" and more \"what the institution did to allow the model to do that\"."
%}

## Where the field disagrees

Lazar thinks the answer is to lean harder into procedural legitimacy: build institutions where contestation is possible, and stop pretending technical artifacts can substitute for due process.

Vredenburgh thinks the answer is finer-grained: a taxonomy of *kinds* of explanation, each appropriate to a different contestability context. I find myself drawn to Vredenburgh's view but worried about its scalability. A taxonomy is only useful if it can be operationalized at the speed at which decisions are made.
...
4. Be honest that some questions ("did the model make the morally right decision?") are not technical questions and never will be.

That last point is, I suspect, the hardest one to swallow. The field has spent a decade selling itself as the discipline that would make models *trustworthy*. The honest answer is that trust is not a model property; it is a relational achievement between an institution and its subjects.[^2] We can help. We cannot deliver.

[^2]: See also Mireshghallah et al.[^mireshghallah2024] on a parallel point about privacy: trust is a property of how data flows, not of any single artifact.

[^mireshghallah2024]: Mireshghallah, N. et al. (2024). *Can LLMs Keep a Secret? Testing Privacy Implications of LLMs via Contextual Integrity Theory.* ICLR. [arXiv](https://arxiv.org/abs/2310.17884).

That's a less impressive pitch. It's also a more honest one. I'd rather be in a field doing modest work well than in one promising trust as a technical deliverable.
